...
impl DeleteScore {
    pub fn create(args: &[String], optional_args: &HashMap<String, String>) -> Result<DeleteScore, String> {

        for (key, _) in optional_args {
            if !ADMITTED_OPTIONAL_ARGUMENTS.contains(&key.as_str()) {
                return Err(format!("Unknown optional command for delete-score {}.", key));
            }
        }
...
mod test {
    ...
    fn test_create_no_arguments() {
        ...
        assert_eq!(result.unwrap_err(), "Invalid number of arguments for delete-score.");
    }
    ...
    fn test_create_no_admitted_arguments() {
        ...
        let result = DeleteScore::create(&args, &optional_args);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Unknown optional command for delete-score --unknown.");
    }
}